...
3 papers published in 2021, one looking at variability in the analysis of replications, another one at opinions about different social policies, and one OBHDP looking at the effects of scientists’ gender and professional status on verbosity during group meetings. Two papers in 2020, one looking at variability in the analysis of fmri (neuroimaging datasets), another in applied microeconomics. One paper from 2019 that evaluated the extent to which clinical psychologists analyzing the same experience sampling data come to different conclusions about symptoms to target for treatment. And one really famous study in 2018, that looked at discrimination against darker skin toned soccer players, and arguably started the recent crowd science movement in behavioral science. 
...
What is the rationale of crowdsourced data analysis - why should we be convinced by its findings?
...
The true weight, and the mean of all guesses was 1197 pounds. This may seem unsurprising now, but it wasn't back then. Back then, the fact that crowds were wise was a surprise. Crowds mostly had negative associations, were referred to as mobs, and there was heated intellectual debate about restricting the right to vote to intellectuals, rather than democratizing society. 
...
I'll mention some benefits, most of which are intimately related with each other, some of them will overlap. 
...
Promote inclusivity, include researchers from underrepresented institutions. This comes with all the benefits of diversity. 
Allows us to leverage under-used resources and skills. We can't all be an expert on everything. If we recruit a diverse sample of data analysts, some may have expertise on a certain statistical method that is most appropriate for the data at hand, whereas others do not
...
Helps to acknowledge that results are very variable, and depend on idiosyncratic choices we have to make among multiple valid approaches, sometimes even choices we make unconsciously, such as the version of statistical software we rely on or the different defaults in for instance SPSS and R: the many-analyst approach does not *create* variability in results, it simply *exposes* it. that should give us pause and epistemic humility when we assess conventional scientific research: the particular outcome you're looking at is one from a distribution of outcomes that could have been possible, if the analyst had made different choices
...
Can we explain the strength of the evidence obtained by the individual decisions researchers make? Does the choice to include or exclude a certain control variable, to run a certain statistical model, to exclude or include certain participants, impact the results? Do people who obtain positive findings use more controls or exclude more participants? Do people who obtain negative findings use more complex statistical models?

Shows us *the garden of forking paths*
--
...
To what extent does a finding hold across many different analytic specifications? Do the results come it consistently positive, or consistently negative, across different independent analyses?
--
...
● Diverging conclusions decrease confidence in the analytical robustness of a finding and prompt an examination of the reasons for the divergence"
...
Lofty statements and worthy goals - empirical claim, but no evidence yet. that's what we'll be looking at today.
---
class: inverse, center, middle
name: method
...
Importantly, in both the single-analyst and the multi-analyst conditions we emphasize that there are a lot of choices to be made. 
...
`r icon_style(icons::fontawesome("sitemap"), scale = 1.5, fill = "#035AA6")`   $~$ Crowdsourced data analysis has **many worthy uses**, but...  
...
`r icon_style(icons::fontawesome("hand-holding-usd"), scale = 1.5, fill = "#035AA6")` $~~$ It's highly **resource-intensive**  
--

`r icon_style(icons::fontawesome("exclamation-triangle"), scale = 1.5, fill = "#035AA6")` $~~$ Lay consumers may respond negatively to **variability** and  **lack of consensus**     
--

`r icon_style(icons::fontawesome("question-circle"), scale = 1.6, fill = "#035AA6")` $~~$ Surprisingly, even **multiple, consistent estimates may backfire**   
--
