...
<!-- In line with social norm theory [@miller2016], we expect an individual who observes a crowd of scientists reach the same qualitative conclusion to be more likely to conform in opinion, and be swayed by the scientific findings, than an individual who observes the conclusion of a single scientist. Although observing several differing estimates (rather than a single estimate) may decrease lay confidence in the precise average estimate, it should increase beliefs about the reported phenomenon and decrease perceptions of bias and error. However, when laypeople observe several scientists independently come to differing qualitative conclusions, we expect the multi-analyst method to backfire; when the results generated by independent analyses vary widely and lack consensus in their qualitative conclusions, we expect a decrease in the sway of scientific findings.   -->
<!-- In sum, our preregistered hypotheses are as follows: when laypeople observe multiple consistent (inconsistent) estimates from a crowd of independent scientists, we expect -- compared to a single, non-crowdsourced estimate -- higher (lower) posterior beliefs and credibility of the results, and lower (greater) ratings of bias and error. In addition, we expect that observing differing estimates decreases confidence in the precise average parameter estimate in both multi-estimate conditions.  -->
...
get_d <- function(Predictor, Multi_Condition = "Consistent") {
  linear_mod <- lm(
    pull(df, {{ Predictor }}) ~ Condition + Prior_beliefs, data = df)
  ...
  ifelse(Multi_Condition == "Consistent",
         parse_number(apa_results$table$statistic[2])/sqrt(499),
         parse_number(apa_results$table$statistic[3])/sqrt(500)) 
}
...
t_test <- apa_print(t.test(df_multiconsistent$Final_beliefs,
                           df_multiconsistent$Prior_beliefs, 
                           paired = TRUE))

cohens_d <- parse_number(gsub(".*=", "", t_test$statistic)) / sqrt(499)
...
# Post-hoc t-test
df_multi_inconsistent <- df %>% 
  filter(Condition == "Multi-inconsistent")

t_test <- apa_print(t.test(df_multi_inconsistent$Final_beliefs,
                           df_multi_inconsistent$Prior_beliefs, 
                           paired = TRUE))
...
t_test <- apa_print(t.test(df_single$Final_beliefs,
                           df_single$Prior_beliefs, 
                           paired = TRUE))
...
p1b <- p1 + 
  ggtitle("Differences from the single-analyst condition") +
